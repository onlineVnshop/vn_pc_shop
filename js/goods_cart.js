...
$('#balance').click(function(){
    if($('#content-detail input:checked').length === 0){
        alert('请选择商品');
        return;
    }else{
        var goods_ids = [];
        var goods_id=$('#content-detail input:checked').parent().parent();
        for(var i = 0; i < goods_id.length;i++){
            goods_ids.push(goods_id[i].id.substr(6));
        }
        localStorage.setItem('goods_id',goods_ids);
        var totalPrice = $('#totalPrice i').text().substr(1);
        location.href='goods_balance.html?total='+totalPrice;
    }
});